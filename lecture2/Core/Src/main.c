...
static void blink() {
	...
	if (Tick > last_blink_tick + DELAY){
		last_blink_tick = Tick;
		//every 300 ms do this
...
static void button1() {
	...
	if (Tick > last_button_tick + BUTTON1_DELAY){
...
static void reg_button(){

	static uint16_t debounce = 0xFFFF;
	static uint32_t last_button_tick1;


	// vzorkuj button kazdych 5 ms
	if (Tick > last_button_tick1 + BUTTON_SHIFT){

		last_button_tick1 = Tick;
		//bitovy posuv
		debounce <<= 1;

		// jaka je hodnota aktualni buttonu?
		uint32_t s1 = LL_GPIO_IsInputPinSet(S1_GPIO_Port, S1_Pin);

		// na posledni misto dej hodnotu
		debounce += s1;

		// jestli je na prvnim miste 1 a zbytek 0 ON
		if (debounce == 0x8000){
			LL_GPIO_SetOutputPin(LED2_GPIO_Port, LED2_Pin);
			off_time = Tick + ONE_SEC;
		}

		// zhasinani LEDky po dobu 1s
		if (Tick > off_time) {
			LL_GPIO_ResetOutputPin(LED2_GPIO_Port, LED2_Pin);
		}
